...
std::map<std::string,int> heaviness;
  ...
  for(int i=0;i<4;i++){
  	cache1.push_back(v[i]);
  	cache2.push_back(v[i+4]);
  }
  ...
  std::cout<<"Enter vnf name and alive times of each and user[i] 1 or 2 requesting file\n";
  for(int i=0;i<no;i++){
    std::cin>>req[i];
    std::cin>>t[i];
    std::cin>>user[i];
  ...
  CsmaHelper p2p;
    ...
    NodeContainer c1 = NodeContainer(nC,r1,ncache1,nu1);
 ...
 int poolflag=0,cacheflag=0;
  ...
  if(firewallflag==0){
    ...
    for(int i=0;i<no;i++){
      ...
      sourceApps = source.Install (c1.Get(location1[req[i]]));
      sourceApps.Start (Seconds (0.0));
        sourceApps.Stop (Seconds (t[i]));
      PacketSinkHelper sink ("ns3::TcpSocketFactory",InetSocketAddress (Ipv4Address::GetAny (), port));
      ApplicationContainer sinkApps;
       sinkApps = sink.Install (c1.Get(0));
      ...
      else
      {
      	...
      	source.SetAttribute ("MaxBytes", UintegerValue (maxBytes));
      ApplicationContainer sourceApps;
      sourceApps = source.Install (c2.Get(location2[req[i]]));
      sourceApps.Start (Seconds (0.0));
        sourceApps.Stop (Seconds (t[i]));
      PacketSinkHelper sink ("ns3::TcpSocketFactory",InetSocketAddress (Ipv4Address::GetAny (), port));
      ApplicationContainer sinkApps;
      sinkApps = sink.Install (c2.Get(0));
       sinkApps.Start (Seconds (0.0));
        sinkApps.Stop (Seconds (t[i]));
            ...
            sourceApps1.Start (Seconds (t[i]/2+1));
              ...
              sinkApps1.Stop (Seconds (t[i]));
                	...
                	sourceApps.Start (Seconds (0.0));
              		...
              		PacketSinkHelper sink ("ns3::TcpSocketFactory",InetSocketAddress (Ipv4Address::GetAny (), port));
              		ApplicationContainer sinkApps;
              		sinkApps = sink.Install (ncache2);
              		sinkApps.Start (Seconds (0.0));
              		sinkApps.Stop (Seconds (t[i]/2));
              		BulkSendHelper source1 ("ns3::TcpSocketFactory",
            	             InetSocketAddress (addru2, port1));
              		source1.SetAttribute ("MaxBytes", UintegerValue (maxBytes));
            ApplicationContainer sourceApps1;
             sourceApps1 = source1.Install (ncache2);
             sourceApps1.Start (Seconds (t[i]/2+1));
              sourceApps1.Stop (Seconds (t[i]));

              PacketSinkHelper sink1 ("ns3::TcpSocketFactory",InetSocketAddress (Ipv4Address::GetAny (), port1));
              ApplicationContainer sinkApps1;
              sinkApps1 = sink1.Install (nu2);
              sinkApps1.Start (Seconds (t[i]/2+1));
              sinkApps1.Stop (Seconds (t[i]));
