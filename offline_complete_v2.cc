...
int main(int argc, char  *argv[])
{
  ...
  for(int i=1;i<=40;i++){
    ...
    location1["v"+NumberToString(i)]=2;
    location2["v"+NumberToString(i)]=2;
  ...
  for(int i=0;i<cachesize;i++){
    ...
    location1[v[i]] = 3;
    cache2.push_back(v[i+cachesize]);
    location2[v[i+cachesize]] = 3;
  }
  ...
  if(firewallflag==0){
      ...
      if(user[i]==1){
        ...
        std::cout<<(heaviness[req[i]]+1)*t[i]*100<<std::endl;
         source.SetAttribute ("SendSize", UintegerValue ((heaviness[req[i]]+1)*t[i]*100));
      ...
      sourceApps.Start (Seconds (0.0));
        ...
        source1.SetAttribute ("SendSize", UintegerValue ((heaviness[req[i]]+1)*t[i]*100));
