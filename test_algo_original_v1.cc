...
std::map<std::string,int> delaycache;
std::map<std::string,int> throughputcache;
std::map<std::string,int> bandwidthcache;
std::map<std::string,int> delaypool;
std::map<std::string,int> throughputpool;
std::map<std::string,int> bandwidthpool;
...
if(userflag==1){
  ...
  if(flag==1){
    ...
    if(cache1.size()==cachesize){
      int pos1 = 0;
      minrank = rank[cache1[0]];
      for(int i=1;i<cachesize;i++){
        if(rank[cache1[i]]<minrank){
          minrank  = rank[cache1[i]];
          pos1 = i;
        }
      }
      std::string temp = cache1[pos1];
      cache1[pos1] = cache1[cachesize-1];
      cache1[cachesize-1] = temp;

      location1[cache1[cache1.size()-1]] = 2;
      cache1.pop_back();
      ...
      for(int i=0;i<cachesize;i++){
        rank[cache1[i]] = rank[cache1[i]] - minrank;
      }
    ...
    std::cout<<"vnf present in cache moved to front\n";
           int temprank = sqrt(bandwidthpool[cache1[pos]]*bandwidthpool[cache1[pos]]+delaypool[cache1[pos]]*delaypool[cache1[pos]]+throughputpool[cache1[pos]]*throughputpool[cache1[pos]]);
  int temprank1 =  sqrt(bandwidthcache[cache1[pos]]*bandwidthcache[cache1[pos]]+delaycache[cache1[pos]]*delaycache[cache1[pos]]+throughputcache[cache1[pos]]*throughputcache[cache1[pos]]);
    rank[cache1[pos]] = (temprank/temprank1)*cloneweight + priority[cache1[pos]]*prweight; 
    hitrate++;
  }}else{
  ...
  if(flag==1){
    ...
    if(cache2.size()==cachesize){
      int pos1 = 0;
      minrank = rank[cache2[0]];
      for(int i=1;i<cachesize;i++){
        if(rank[cache2[i]]<minrank){
          minrank  = rank[cache2[i]];
          pos1 = i;
        }
      }
      std::string temp = cache2[pos1];
      cache2[pos1] = cache2[cachesize-1];
      cache2[cachesize-1] = temp;

      location2[cache2[cache2.size()-1]] = 2;
      cache2.pop_back();
      std::vector<std::string>::iterator it;
      ...
      location2[s] = 3;
      for(int i=0;i<cachesize;i++){
        rank[cache2[i]] = rank[cache2[i]] - minrank;
      }
      std::cout<<"cache was full least rank removed\n";
    ...
    std::cout<<"vnf present in cache moved to front\n";
        int temprank = sqrt(bandwidthpool[cache2[pos]]*bandwidthpool[cache2[pos]]+delaypool[cache2[pos]]*delaypool[cache2[pos]]+throughputpool[cache2[pos]]*throughputpool[cache2[pos]]);
  int temprank1 =  sqrt(bandwidthcache[cache2[pos]]*bandwidthcache[cache2[pos]]+delaycache[cache2[pos]]*delaycache[cache2[pos]]+throughputcache[cache2[pos]]*throughputcache[cache2[pos]]);
    rank[cache2[pos]] = (temprank/temprank1)*cloneweight + priority[cache2[pos]]*prweight; 
    hitrate++;
  }
  }
}
...
int main(int argc, char  *argv[])
{
  ...
  cache2.clear();
	for(int i=0;i<v.size();i++){
		clonetime[v[i]] = (rand()%3+1)*10;
    priority[v[i]] = (rand()%14)*5;
  ...
  int temprank = sqrt(bandwidthpool[v[0]]*bandwidthpool[v[0]]+delaypool[v[0]]*delaypool[v[0]]+throughputpool[v[0]]*throughputpool[v[0]]);
  int temprank1 =  sqrt(bandwidthcache[v[0]]*bandwidthcache[v[0]]+delaycache[v[0]]*delaycache[v[0]]+throughputcache[v[0]]*throughputcache[v[0]]);
  
  rank[v[0]] = (temprank/temprank1)*cloneweight + priority[v[0]]*prweight; 
    ran[0] = rank[v[0]];
    tempo[0] = v[0];
    minrank = rank[v[0]];
  for(int i=0;i<v.size();i++){
     int temprank = sqrt(bandwidthpool[v[i]]*bandwidthpool[v[i]]+delaypool[v[i]]*delaypool[v[i]]+throughputpool[v[i]]*throughputpool[v[i]]);
  int temprank1 =  sqrt(bandwidthcache[v[i]]*bandwidthcache[v[i]]+delaycache[v[i]]*delaycache[v[i]]+throughputcache[v[i]]*throughputcache[v[i]]);
    rank[v[i]] = (temprank/temprank1)*cloneweight + priority[v[i]]*prweight; 
    ran[i] = rank[v[i]];
    tempo[i] = v[i];
    if(rank[v[i]] < minrank){
      minrank = rank[v[i]];
    }
    std::cout<<rank[v[i]]<<std::endl;
  }
